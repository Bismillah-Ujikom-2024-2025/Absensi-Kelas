<template>
  <div class="container-fluid">
    <div class="row">
      <div class="col-lg-12">
        <h2 class="text-center Riwayat">Riwayat</h2>
        <div class="my-3">
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="(visitor,i) in visitors" :key="i">
                ...
                <td>{{ visitor.siswa.jk }}</td>
                <td>{{ visitor.siswa.kelas }}</td>
                <td>{{ visitor.waktu }}</td>
                <td>{{ visitor.keterangan.keterangan }}</td>
                <td><img :src="visitor.foto" width="100px" height="150px"></td>
              </tr>
            </tbody>
          </table>
        </div>
...
const getKehadiran = async () => {
  const { data, error } = await supabase.from('kehadiran').select(`*, keterangan(*), siswa(*)`)
  if(data) visitors.value = data
  // if(data) {
  //   data = data.maps(data => {
  //     const { data: url } = supabase.storage.from('kehadiran').getPublicUrl(data.foto)
  //     return {
  //       ... data,
